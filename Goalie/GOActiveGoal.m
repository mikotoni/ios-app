...
- (void)updateEarnedPointsByQuery:(CouchQuery *)query {
    ...
    if(enumerator) {
        ...
        if(row) {
            ...
            if(query == _earnedPointsLiveQuery) {
                self.earnedPoints = [row value];
                earnedPoints = [self.earnedPoints floatValue];
                if(earnedPoints > 100)
                    earnedPoints = 100;
                self.completionRate = [NSNumber numberWithFloat:(earnedPoints / 100.0)];
            }
            else if(query == _earnedPointsDailyQuery) {
                self.earnedPointsDaily = [row value];
                earnedPoints = [self.earnedPointsDaily floatValue];
                if(earnedPoints > 100)
                    earnedPoints = 100;
                self.completionRateDaily = [NSNumber numberWithFloat:(earnedPoints / 100.0)];
            }
            else if(query == _earnedPointsWeeklyQuery) {
                self.earnedPointsWeekly = [row value];
                earnedPoints = [self.earnedPointsWeekly floatValue];
                if(earnedPoints > 100)
                    earnedPoints = 100;
                self.completionRateWeekly = [NSNumber numberWithFloat:(earnedPoints / 100.0)];
            }
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context {
    if(object == _earnedPointsLiveQuery || object == _earnedPointsDailyQuery || object == _earnedPointsWeeklyQuery) {
        [self updateEarnedPointsByQuery:object];
    }
...
- (NSNumber *)earnedPointsDaily {
    if(!_earnedPointsDailyQuery) {
        GOGoalieServices *goalieServices = [[GOMainApp sharedMainApp] goalieServices];
//        _earnedPointsDailyQuery = [goalieServices liveQueryForEarnedPointsByActiveGoal:self];
        _earnedPointsDailyQuery = [goalieServices liveQueryForEarnedPointsByActiveGoal:self forDate:[NSDate date]];
        [_earnedPointsDailyQuery start];
        [_earnedPointsDailyQuery addObserver:self forKeyPath:@"rows" options:0 context:NULL];
        self.firstEarnedPointsValue = YES;
    }
    return _earnedPointsDaily;
}
- (NSNumber *)completionRate {
    [self earnedPoints];
    return _completionRate;
}
- (NSNumber *)completionRateDaily {
    [self earnedPointsDaily];
    return _completionRateDaily;
}
